...
export class PantryService {
    ...
    getCurrentInventory(): Observable<PantryLineGrouping[]> {
        return from (this._authService.getAccessToken().then(accessToken => {
            ...
            return this.httpClient.get<PantryLineGrouping[]>(Constants.recipeApi + 'v1/Pantry/0?includeZeroValues=false', { headers: headers }).toPromise();
        }));
    }
    ...
    getProductSummary(productId: number): Observable<PantryLineGrouping[]> {
        return from(this._authService.getAccessToken().then(accessToken => {
            var headers = new HttpHeaders().set('Authorization', `Bearer ${accessToken}`);
            return this.httpClient.get<PantryLineGrouping[]>(`${Constants.recipeApi}v1/Pantry/0/product/${productId}?includeZeroValues=true`, { headers: headers }).toPromise();
        }));
    }
}